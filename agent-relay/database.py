...
This module is the only place that knows about the SQLite writer-lock
transaction versus PostgreSQL row locking; the rest of the application talks
to the models through :mod:`storage`. ``RELAY_DATABASE_URL`` selects the
backend: a ``sqlite://`` URL (the default) uses WAL mode and
``BEGIN IMMEDIATE``; a ``postgresql+psycopg://`` URL uses ordinary
transactions plus ``SELECT ... FOR UPDATE`` locking in :mod:`storage`.
"""
...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    SQLite has no ``FOR UPDATE SKIP LOCKED``, so the starter reserved a
    ``BEGIN IMMEDIATE`` writer lock up front to serialize claims, heartbeats,
    terminal submissions, and recovery across API processes. On PostgreSQL a
    plain transaction is enough: the row-locking queries in ``storage.py``
    (``with_for_update`` / ``skip_locked``) provide the same single-active-lease
    guarantee at row granularity instead of serializing every writer.
    """
...
def lock_for_update(query: Any, *, skip_locked: bool = False) -> Any:
    """Add row locking to ``query`` on PostgreSQL; a no-op on SQLite.

    SQLite lacks ``SELECT ... FOR UPDATE`` and instead relies on the
    ``BEGIN IMMEDIATE`` writer reservation from :func:`immediate_transaction`.
    """
