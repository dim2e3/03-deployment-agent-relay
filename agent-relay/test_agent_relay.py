...
def test_full_task_exchange_sender_sees_completed_status():
    """End-to-end flow against the real app and DB: register two agents,
    send a task, claim and complete it, and confirm the sender observes the
    result once the recipient submits it."""
    with TestClient(main.app) as client:
        sender, sender_headers = register(client, "sender-demo")
        recipient, recipient_headers = register(client, "echo-demo")

        sent = client.post(
            "/api/v1/tasks",
            headers=sender_headers,
            json={"to": recipient["agent_id"], "input": "Reverse this string: agent relay"},
        )
        assert sent.status_code == 201
        task_id = sent.json()["task_id"]
        ...
        claim = client.post(
            "/api/v1/tasks/claim",
            headers=recipient_headers,
            json={"worker_id": "echo-demo-worker", "wait_seconds": 0},
        )
        assert claim.status_code == 200
        claim_data = claim.json()
        reversed_output = claim_data["input"].split(": ", 1)[1][::-1]

        complete = client.post(
            f"/api/v1/tasks/{task_id}/complete",
            headers=recipient_headers,
            json={"claim_token": claim_data["claim_token"], "output": reversed_output},
        )
        assert complete.status_code == 200
        ...
        assert result["output"] == "yaler tnega"
