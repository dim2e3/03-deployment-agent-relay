...
def _existing_task_for_idempotency_key(db: Session, sender_id: str, idempotency_key: str) -> Task | None:
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # SQLite: the BEGIN IMMEDIATE writer lock already serializes the
    # idempotency-key check against the unique constraint. On PostgreSQL two
    # requests can pass the check concurrently, so a race is instead resolved
    # by catching the unique-constraint violation below and returning the row
    # that won.
    with immediate_transaction() as db:
        ...
        if idempotency_key is not None:
            existing = _existing_task_for_idempotency_key(db, sender_id, idempotency_key)
        ...
        try:
            ...
            with db_session() as retry_db:
                existing = _existing_task_for_idempotency_key(retry_db, sender_id, idempotency_key)
...
def claim_one(agent_id: str, worker_id: str | None) -> dict[str, Any] | None:
    with immediate_transaction() as db:
        ...
        task = db.scalar(lock_for_update(query, skip_locked=True))
        if task is None:
            return None
...
def heartbeat(task_id: str, agent_id: str, claim_token: str) -> str:
    with immediate_transaction() as db:
        task = db.scalar(lock_for_update(select(Task).where(Task.id == task_id)))
